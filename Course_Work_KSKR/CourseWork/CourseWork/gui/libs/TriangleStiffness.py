...
class TriangleStiffness(Polygon, ABC):
    ...
    '''
    Метод позволяющий определить локальную матрицу жесткости для данного конечного элемента
    '''
    ...
    '''
    Метод определяющий порядок следования узлов (против часовой стрелки) для применения в матрице
